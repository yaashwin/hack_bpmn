...
interface UploadStatus {
  video: 'success' | 'error' | null;
}

interface IsUploading {
  video: boolean;
}
...
interface UploadPageProps {
  onNavigateToSummary: (data: SummaryData) => void;
}
...
function App(): JSX.Element {
  const [currentPage, setCurrentPage] = useState<PageType>(PAGES.UPLOAD);
  const [summaryData, setSummaryData] = useState<SummaryData | null>(null);
  
  const navigateToSummary = (data: SummaryData): void => {
    setSummaryData(data);
    setCurrentPage(PAGES.SUMMARY);
  };
...
// Upload Page Component
function UploadPage({ onNavigateToSummary }: UploadPageProps): JSX.Element {
  const [videoFile, setVideoFile] = useState<File | null>(null);
  const [videoURL, setVideoURL] = useState<string | null>(null);
  const [textContent, setTextContent] = useState<string>("");
  const [textFileName, setTextFileName] = useState<string>("");
  const [uploadStatus, setUploadStatus] = useState<UploadStatus>({ video: null });
  const [isUploading, setIsUploading] = useState<IsUploading>({ video: false });
  const [isProcessing, setIsProcessing] = useState<boolean>(false);
  const [file, setFile] = useState<File | null>(null);
  // API call to upload video file
  const uploadVideoToBackend = async (file: File): Promise<void> => {
    setIsUploading(prev => ({ ...prev, video: true }));
    setUploadStatus(prev => ({ ...prev, video: null }));
  ...
  const handleSendText = async (): Promise<void> => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/upload/text-binary', fileTextContent, {
        headers: {
          'Content-Type': 'text/plain',
        },
      });

      const result: SummaryData = await response.data;
      console.log('Processing response:', result);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTextFileName(uploadFile.name);
    const reader = new FileReader();
    reader.onload = (event: ProgressEvent<FileReader>): void => {
      const content = event.target?.result as string;
      setTextContent(content);
...
// Summary Page Component
function SummaryPage({ data, onBack }: SummaryPageProps): JSX.Element {
  ...
  const downloadResponseFiles= async (filename:string) => {
    try {
      const response = await fetch(`http://localhost:3001/api/upload/download/${filename}`);
      if (!response.ok) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {Object.entries(data.data.generatedFiles).map(([type, filename]) => (
                  <div key={type} className="bg-gray-100 rounded-lg p-3">
                    ...
                    <div className="text-gray-800 truncate">{filename}</div>
                    <div><button onClick={()=>{downloadResponseFiles(filename)}}>Download</button></div>
                  </div>
                ))}
              </div>
...
// Status Indicator Component
function StatusIndicator({ status, isUploading, type }: StatusIndicatorProps): JSX.Element | null {
  if (isUploading) {
    return (
      <div className="flex items-center gap-2 text-blue-600">
        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600" />
